...
describe('filter', () => {
  test('byVariableName matches on a common variable name', () => {
    const standards = library.filter().byVariableName('atmospheric_pressure').standards;
    expect(standards.map((s) => s.name)).toContain('air_pressure_at_mean_sea_level');
  });
  ...
  test('chained filters compose', () => {
    ...
    const filtered = library.filter().byUnit('Pa').byIoosCategory('Meteorology').standards;
